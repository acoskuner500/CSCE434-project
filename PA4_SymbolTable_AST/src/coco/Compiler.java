...
// Grammar Rules ==============================================================
    ...
    private Designator designator () {
        ...
        Stack<Pair<Expression, Token>> idxs = new Stack<Pair<Expression, Token>>();

        while (have(Token.Kind.OPEN_BRACKET)) {
            Token start = expectRetrieve(Token.Kind.OPEN_BRACKET);
            idxs.push(new Pair<Expression, Token>(relExpr(), start));
        ...
        while (!idxs.isEmpty()) {
            Expression idxVal = idxs.peek().getFirst();
            Token start = idxs.peek().getSecond();

            if (arrIdx == null) {
                arrIdx = new ArrayIndex(start.lineNumber(), start.charPosition(), sym.get(0), idxVal);
            } else {
                arrIdx = new ArrayIndex(start.lineNumber(), start.charPosition(), arrIdx, idxVal);
            }
            idxs.pop();
        }
        ...
        if (arrIdx == null) {
            desig = new Designator(ident.lineNumber(), ident.charPosition(), (sym == null) ? null : sym.get(0));
        } else {
            desig = new Designator(ident.lineNumber(), ident.charPosition(), arrIdx);
        }
    ...
    private ArrayList<VariableDeclaration> varDecl () {
        ...
        do {
            ...
            if (s != null) {
                vars.add(new VariableDeclaration(identTok.lineNumber(), identTok.charPosition(), s));
            }
    ...
    private Type typeDecl () {
        ...
        return new ArrayType(baseType, dims);
    }
    ...
    private  Assignment assign () {
        ...
        // System.out.println("ASSIGNING TO: " + ident.)
        ...
        if (have(NonTerminal.ASSIGN_OP)) {
            ...
            switch (assignOp.kind()) {
                case ADD_ASSIGN:
                    source = new Addition(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                case SUB_ASSIGN:
                    source = new Subtraction(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                case MUL_ASSIGN:
                    source = new Multiplication(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                case DIV_ASSIGN:
                    source = new Division(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                case MOD_ASSIGN:
                    source = new Modulo(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                case POW_ASSIGN:
                    source = new Power(assignOp.lineNumber(), assignOp.charPosition(), ident, source);
                    break;
                default:
            }

            return new Assignment(ident.lineNumber(), ident.charPosition(), ident, source);
            ...
            switch (unaryOp.kind()) {
                case UNI_INC:
                    if (identType instanceof IntType) {
                    ...
                    } else if (identType instanceof ArrayType) {
                        ...
                        // TODO: TYPE ERROR (CAN ONLY CALL ++ ON NUMERICAL TYPES)
                        return null;
                    }
                case UNI_DEC:
                    if (identType instanceof IntType) {
    ...
    private FunctionCall funcCall () {
        ...
        FunctionCall ret = new FunctionCall(start.lineNumber(), start.charPosition(), overloads, new ArgumentList(paramStart.lineNumber(), paramStart.charPosition(), params));
    ...
    private Expression relExpr () {
        ...
        while (have(NonTerminal.REL_OP)) {
            ...
            ret = new Relation(ret.lineNumber(), ret.charPosition(), ret, op.lexeme(), second); 
        }
    ...
    private Expression addExpr () {
        ...
        while (have(NonTerminal.ADD_OP)) {
            ...
            switch (op.kind()) {
                case ADD:
                    ret = new Addition(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                case SUB:
                    ret = new Subtraction(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                case OR:
                    ret = new LogicalOr(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                default:
            }
        }
    ...
    private Expression multExpr () {
        ...
        while (have(NonTerminal.MUL_OP)) {
            ...
            switch (op.kind()) {
                case MUL:
                    ret = new Multiplication(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                case DIV:
                    ret = new Division(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                case MOD:
                    ret = new Modulo(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                case AND:
                    ret = new LogicalAnd(ret.lineNumber(), ret.charPosition(), ret, second);
                    break;
                default:
            }
        }
    ...
    private Expression powExpr () {
        ...
        while (accept(Token.Kind.POW)) {
            ...
            ret = new Power(ret.lineNumber(), ret.charPosition(), ret, second);
        }
    ...
    private Expression groupExpr() {
        if (have(NonTerminal.LITERAL)) {
            ...
            return new LogicalNot(line, pos, expr);
